...
(function(CS) {
    ...
    symbolVis.prototype.init = function(scope, elem) {
        ...
        function myCustomDataUpdateFunction(data) {
            ...
            if (!data || !data.Data || !Array.isArray(data.Data) || data.Data.length >= 2) {
                ...
                if (newObject.Data0.length > 0) {
                    ...
                    var result = getMonthsOfYear(2024);
                    // console.log(result);
                    ...
                    for (var i = 1; i <= monthsData.number; i++) {
                        ...
                        if (i <= monthNow) {
                            for (var j = 0; j < data.Data[0].Values.length; j++) {
                                var valueMonth = new Date(data.Data[0].Values[j].Time).getMonth() + 1;
                                if (valueMonth === i) {
                                    var valueInTons = data.Data[0].Values.length > 0 ? parseFloat(("" + data.Data[0].Values[j].Value).replace(",", ".")).toFixed(scope.config.decimalPlaces) : 0;

                                    if (scope.config.convert === 'kilotoneladas') {
                                        newObject.value1 = (valueInTons * 0.001).toFixed(scope.config.decimalPlaces); // Conversión a kilotoneladas
                                    } else {
                                        newObject.value1 = valueInTons; // Valores en toneladas
                                    }
                            ...
                            for (var k = 0; k < data.Data[1].Values.length; k++) {
                                var valueMonth = new Date(data.Data[1].Values[k].Time).getMonth() + 1;
                                if (valueMonth === i) {
                                    var valueInTons = data.Data[1].Values.length > 0 ? parseFloat(("" + data.Data[1].Values[k].Value).replace(",", ".")).toFixed(scope.config.decimalPlaces) : 0;

                                    if (scope.config.convert === 'kilotoneladas') {
                                        newObject.value2 = (valueInTons * 0.001).toFixed(scope.config.decimalPlaces); // Conversión a kilotoneladas
                                    } else {
                                        newObject.value2 = valueInTons; // Valores en toneladas
                                    }
        ...
        function getMonthsOfYear(numYear) {
            ...
            if (leapYear) {
                // Si es bisiesto, eliminar el elemento "Año Bisiesto" si existe
                var index = monthNames.indexOf("Año Bisiesto");
                if (index !== -1) {
                    monthNames.splice(index, 1);
                }
            }
            var monthsData = {
                number: 12,
        ...
        function myCustomConfigurationChangeFunction(data) {
            if (chart) {
                ...
                if (scope.config.convert === 'kilotoneladas') {
                    // Si la configuración de conversión está en 'kilotoneladas', actualiza los valores del gráfico a kilotoneladas.
                    chart.dataProvider = newArray.map(function(item) {
                        return {
                            timestamp: item.timestamp,
                            value1: (parseFloat(item.value1) * 0.001).toFixed(scope.config.decimalPlaces), // Conversión a kilotoneladas
                            value2: (parseFloat(item.value2) * 0.001).toFixed(scope.config.decimalPlaces), // Conversión a kilotoneladas
                        };
                    });
                } else {
                    // De lo contrario, los valores se mantienen en toneladas.
                    chart.dataProvider = newArray;
                }
